$(function(){
    mise_en_place();
    $("#banniere").click(function(){location.assign("index.html")});
});
...
function mise_en_place(){
    //mise en place du bloc paramfragile
        ...
        $("#valeur_fragile").change(function(){modifier_paramfragile(DEFAUT);});
    ...
    //reinitialisation
    $("#reinitialiser").click(function(){reinitialiser();});
}
...
{
    if($("#valeur_fragile").val() == defaut){$("#defaut_fragile").hide();}
    else {
                                     ...
                                     modif_absolu_mi, val_defaut_absolu_mi)
{
    ...
    if (is_defaut_relatif_ms){ contenu = contenu.concat(' (par défaut)');}
    else {contenu = contenu.concat(' (modifiée)');}
    ...
    for(var i = 0.5; i<= 10; i += 0.5)
        ...
        if (i == val_relatif_ms){contenu = contenu.concat('"selected="selected')};
    ...
    if (is_defaut_absolu_ms){ contenu = contenu.concat(' (par défaut)');}
    else {contenu = contenu.concat(' (modifiée)');}
    ...
    if (0 == val_absolu_ms){contenu = contenu.concat('"selected="selected')};
    contenu = contenu.concat('">');
    ...
    for(var i = 20; i<= 1000; i += 5)
        ...
        contenu = contenu.concat('<option value="');
        contenu = contenu.concat(i);
        if (i == val_absolu_ms){contenu = contenu.concat('"selected="selected')};
        contenu = contenu.concat('">');
        contenu = contenu.concat(i);
    ...
    if (is_defaut_relatif_mi){ contenu = contenu.concat(' (par défaut)');}
    else {contenu = contenu.concat(' (modifiée)');}
    ...
    for(var i = 0.5; i<= 10; i += 0.5)
        ...
        contenu = contenu.concat('<option value="');
        contenu = contenu.concat(i);
        if (i == val_relatif_mi){contenu = contenu.concat('"selected="selected')};
        contenu = contenu.concat('">');
        contenu = contenu.concat(i);
        contenu = contenu.concat(' mg/kg</option>');
        }
    ...
    if (is_defaut_absolu_mi){ contenu = contenu.concat(' (par défaut)');}
    else {contenu = contenu.concat(' (modifiée)');}
    ...
    contenu = contenu.concat('<option value="');
    contenu = contenu.concat(0);
    if (0 == val_absolu_mi){contenu = contenu.concat('"selected="selected')};
    contenu = contenu.concat('">');
    contenu = contenu.concat("Aucune </option>");
    ...
    for(var i = 20; i<= 1000; i += 5)
        ...
        contenu = contenu.concat('<option value="');
        contenu = contenu.concat(i);
        if (i == val_absolu_mi){contenu = contenu.concat('"selected="selected')};
        contenu = contenu.concat('">');
        contenu = contenu.concat(i);
        contenu = contenu.concat(' mg</option>');
        }
...
{
    $(String("#valeur_relative_MS_").concat(produit_ID)).change(function(){bloc_modifie(produit_ID)});
    $(String("#valeur_absolue_MS_").concat(produit_ID)).change(function(){bloc_modifie(produit_ID)});
    $(String("#valeur_relative_MI_").concat(produit_ID)).change(function(){bloc_modifie(produit_ID)});
    $(String("#valeur_absolue_MI_").concat(produit_ID)).change(function(){bloc_modifie(produit_ID)});
}
